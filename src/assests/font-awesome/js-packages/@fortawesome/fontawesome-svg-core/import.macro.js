...
const {
  FALegacyStyleIds,
} = require("@fortawesome-internal-tools/fontawesome-icons/legacy");
...
function replace({ macroName, license, references, state, babel, source }) {
  references.forEach((nodePath) => {
    const { iconName, style, family } = resolveReplacement({
      nodePath,
      babel,
      state,
      macroName,
...
function resolveReplacementIcon({ nodePath, babel, state, macroName }) {
  ...
  const namePropIndex = properties.findIndex(
    (prop) => "name" === prop.key.name
  );
  ...
  const stylePropIndex = properties.findIndex(
    (prop) => "style" === prop.key.name
  );
  ...
  const familyPropIndex = properties.findIndex(
    (prop) => "family" === prop.key.name
  );
...
function getStringLiteralPropertyValue(t, parentPath, property) {
  ...
  if (
    !(
      "object" === typeof parentPath &&
      "function" === typeof parentPath.buildCodeFrameError
    )
  ) {
    throw Error(
